 ...
 */
static void keys_for_each_wrapper(map_entry_t entry,
				  struct map_for_each_entry *data)
{
	struct for_each_k_wrapper *unwrapped_data =
		(struct for_each_k_wrapper *)data;
...
{
	struct for_each_k_wrapper wrapper = (struct for_each_k_wrapper){
		.func = &keys_for_each_wrapper,
		.key_data = data,
	};
